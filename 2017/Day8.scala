/*val in = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""
*/

val in = """v inc 523 if t == 6
...
val names = (for(l <- lines) yield l.split("\\s+")(0)).distinct
val registers = scala.collection.mutable.Map() ++ (for(n <- names) yield (n, 0)).toMap
...
lines.foreach((l) => {
	val split = l.split("\\s+")
	val name = split(0)
	val dir = if(split(1) == "inc") 1 else -1
	val am = split(2).toInt
	val cwhich = split(4)
	val cop = split(5)
	val cam = split(6).toInt

	val cRegVal = registers(cwhich)
	...
	registers(name) = registers(name) + (if(cond) am * dir else 0)
	highest = math.max(highest, registers.map(_._2).max)
})

println(registers.map(_._2).max)
println(highest)
