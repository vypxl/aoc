val in = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
val phrases = in.split("\n").toList.map(_.split("\\s+").toList)

println(phrases.count(x => x.distinct == x))
println(phrases.count(x => x.map(_.sorted).distinct == x.map(_.sorted)))
